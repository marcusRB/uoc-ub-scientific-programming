...
```{r}
# Mostramos gráficamente con barras
```
...
ggplot(combo_hairloss, aes(x = Stress, y = Mean_Hair_Loss, fill = Stress)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~`Nutritional Deficiencies`) +
  labs(title = "Caída de cabello promedio por deficiencia nutricional y nivel de estrés",
       x = "Nivel de estrés", y = "Proporción con caída de cabello") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
```{r}
ggplot(HairFallH, aes(x = factor(`Hair Loss`, labels = c("Sí", "No")), y = Age, fill = factor(`Hair Loss`))) +
  geom_boxplot() +
...
```{r}
ggplot(HairFallH, aes(x = factor(`Hair Loss`, labels = c("No", "Sí")), y = Age, fill = factor(`Hair Loss`))) +
  geom_boxplot() +
  labs(title = "Relación entre la edad y la caída de cabello",
       x = "Caída de cabello", y = "Edad") +
  theme_minimal()
...
# Convertir a formato largo para el heatmap
...
ggplot(heat_data, aes(x = `Hair_Loss`, y = `Medical Conditions`, fill = count)) +
  ...
  labs(title = "Mapa de calor: condiciones médicas y caída de cabello",
       x = "Caída de cabello (0=No, 1=Sí)", y = "Condición médica") +
  theme_minimal()
```
